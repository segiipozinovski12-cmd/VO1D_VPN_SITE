...
def _explicit_ru_label(uri):
    try:
        frag=urllib.parse.unquote(uri.split("#",1)[1] if "#" in uri else "")
        upper=frag.upper()
        return ("🇷🇺" in frag or "俄罗斯联邦" in frag or "俄罗斯" in frag or
                "[RU]" in upper or upper.startswith("RU ") or " RUSSIA" in upper)
    except Exception:
        return False
...
class CommunityPool:
    ...
    def _fetch_country(self,cc):
        ...
        decoded=raw.decode("utf-8","ignore") if cc=="RU" else _b64decode_text(raw.decode())
        unique=[]
        seen=set()
        ...
        if cc=="RU":
            unique.sort(key=lambda u:(
                _source_latency_ms(u) if _source_latency_ms(u) is not None else 999999,
                -_candidate_score(u,cc)
            ))
        else:
        ...
        if cc=="RU":
            measured.sort(key=lambda pair:(
                _source_latency_ms(pair[0]) if _source_latency_ms(pair[0]) is not None else 999999,
                pair[1]
            ))
